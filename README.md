...
> **Personal version-controlled distribution.** To install or roll back to a
> specific release, start with [`INSTALL-VERSION.md`](INSTALL-VERSION.md). Release
> policy and update procedures are documented in
> [`VERSIONING.md`](VERSIONING.md).

**A one-page, read-only operator console for your AI tool stack.**

Skynet OS is for the operator running a multi-tool stack — Claude Code, ChatGPT / Codex, OpenRouter, Pinecone, Obsidian, OpenClaw — who has no honest way of knowing how much they actually use any of it. Skynet OS reads what's already on your disk, never writes to source tool data directories, and renders one scrollable dashboard with subscriptions, token spend, skills ROI, a 3D memory graph, and a daily **Dream** review that prescribes the four highest-impact things to fix today.
...
```
Install Skynet OS for me. The Skynet OS files are already on my machine
at ~/code/skynet-mission-control. Ask once for permission, then run all three commands
in order without further prompts:

1. cd ~/code/skynet-mission-control && bun install
2. cd ~/code/skynet-mission-control && bun run setup
3. cd ~/code/skynet-mission-control && bun run dev
...
before continuing. For each key I give you:
   echo "<KEY_NAME>=<value>" >> ~/code/skynet-mission-control/.env.local
...
Once the wizard is open, send this line:
  ...
  ✨ Skynet OS is ready. The wizard is open in your browser — walk
...
A) AUTO-DETECTED — Skynet OS already saw it (Obsidian vaults under
...
B) NEEDS WIRING — supported but the auto-scan needs a key, env
   ...
   to ~/code/skynet-mission-control/.env.local then re-running
...
activity and writes 4 prescriptions to ~/.skynet-mission-control/dreams/ every
...
I'll show you what Skynet OS already sees, what needs a quick
...
- **Dream panel** — four prescription cards (memory / cost / skills / workflow), each with a runnable command you can copy. Source: `~/.skynet-mission-control/dreams/dream-{date}.json`.
...
| Feature                | Status                                             |
| ---------------------- | -------------------------------------------------- |
| `bun create skynet-mission-control` | 🔴 Package not yet on npm — use the files for now |
...
**One rule:** the frontend never reads `~/.claude/` directly. The aggregator script is the only reader on disk. The frontend reads `live-data.json` plus the most recent `~/.skynet-mission-control/dreams/dream-{date}.json`. Everything else is derived.
...
```bash
cd skynet-mission-control        # the folder you unzipped
bun install
bun run dev
```
...
```bash
# macOS
launchctl kickstart -k gui/$(id -u)/com.skynet-mission-control.dream
ls ~/.skynet-mission-control/dreams/            # should contain today's JSON

# Windows (PowerShell)
schtasks /run /tn "Skynet Mission Control Dream"
ls $env:USERPROFILE\.skynet-mission-control\dreams\
```
...
The wizard's final action is **one click** — "Activate now" hands the collected state to the local sidecar, which writes `~/.skynet-mission-control/config.json` and the project-root `.env.local`, then triggers the aggregator so the dashboard is hydrated by the time you land on it. The three-command copy-paste fallback only appears if the sidecar can't be reached.

---

## Connecting to everything

The aggregator and dashboard auto-detect what they can. Anything that needs an API key goes in `skynet-mission-control/.env.local`:

```bash
# skynet-mission-control/.env.local
PINECONE_API_KEY=...
OPENROUTER_API_KEY=...
...
`security dump-keychain` (used to estimate Claude plan tier) will additionally prompt for your **login password** the first time it runs. That's a macOS prompt — Skynet OS never sees the password.
...
Dream is what makes Skynet OS more than a metrics dashboard.
...
The cron installer writes `~/Library/LaunchAgents/com.skynet-mission-control.dream.plist` (macOS) or registers a Task Scheduler job (Windows), and pipes output to `~/.skynet-mission-control/dream-cron.log`. The skill lives at `skills/dream/SKILL.md`; the cron calls `scripts/run-dream.ts`, which reads your chosen engine from `~/.skynet-mission-control/config.json` and dispatches to it. On Linux the installer prints a `crontab` snippet you can install by hand.
...
In short: Dream walks 8 signal buckets (CONVERSATION, COST, SKILLS, MEMORY, SESSION, WORKFLOW, EXTERNAL, BUSINESS), ranks every candidate by `severity × dollar_impact`, and emits the top 4 highest-impact prescriptions as JSON to `~/.skynet-mission-control/dreams/dream-{date}.json`. Each prescription comes with a `claude -p "<command>"` that the dashboard can run via its prescription ID or that you can run manually in your terminal.
...
- ⏳ **`bun create skynet-mission-control`** — distribution package on npm so you can `bun create skynet-mission-control my-os`
- ✅ **Backend sidecar** — auto-started alongside Vite by `bun run dev`. Writes `~/.skynet-mission-control/config.json` from the wizard, runs the aggregator on demand, and powers Dream's one-click prescriptions. See [Backend sidecar](#backend-sidecar-auto-started-by-bun-run-dev) below
...
- `POST /api/install` — writes `~/.skynet-mission-control/config.json` and `.env.local` from the wizard's collected state.
...
- `POST /api/run-fix` — accepts `{ "prescriptionId": "..." }`, resolves it from the latest `~/.skynet-mission-control/dreams/dream-{date}.json`, then streams the stored `claude -p` command back as Server-Sent Events.
...
MIT. See [`LICENSE`](LICENSE). You may use, modify, and redistribute this software
(including in commercial work) subject to the MIT license terms.
