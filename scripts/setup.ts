...
async function main() {
  console.log(`${c.bold}Skynet OS — setup${c.reset}`);
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    ...
    log("");
    log(c.dim + "  Verify after fixing:  launchctl kickstart -k gui/$(id -u)/com.skynet-mission-control.dream" + c.reset);
    log(c.dim + "  Then check:           ls ~/.skynet-mission-control/dreams/   (should contain today's JSON)" + c.reset);
  ...
  if (envs.missing.length > 0) {
    log(c.dim + "  Drop any of these into skynet-mission-control/.env.local to unlock more:" + c.reset);
  ...
  log(`  ${c.dim}  Dream cron        ~/Library/LaunchAgents/com.skynet-mission-control.dream.plist${c.reset}`);
  log("");
