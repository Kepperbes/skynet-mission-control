...
import { fetchLiveChatgptUsage, labelChatgptWindow } from "./chatgpt-usage";
import {
...
// Load .env.local from the project directory so secrets like
...
async function parseJsonls() {
  ...
  function modelFamily(m: string): string {
    ...
    if (/claude.*fable/i.test(m)) return "fable";
    return "other";
  }
...
// DeepSeek — the user's Hermes runtime provider. The platform reports the
// account balance via /user/balance; we surface it as the "limit" the same
// way OpenRouter's credit row works. Key from DEEPSEEK_API_KEY (env or
// .env.local). Missing key -> null so the dashboard simply omits the row.
async function fetchDeepSeekBalance() {
  ...
  try {
    ...
    return {
      is_available: j.is_available ?? null,
...
interface AutomationStat {
  ...
  source?: "cowork" | "codex" | "claude" | "skynet-mission-control";
  meta?: string;
}
...
async function loadAutomations(): Promise<AutomationStat[]> {
  ...
  // ── 4. Dream cron (Skynet Mission Control: launchd on macOS, Task Scheduler on Windows) ──
  try {
    let dreamScheduled: boolean;
    if (IS_WIN) {
      // `schtasks /query` exits non-zero when the task doesn't exist.
      try {
        execSync('schtasks /query /tn "Skynet Mission Control Dream"', { stdio: "ignore", timeout: 4000 });
        dreamScheduled = true;
      } catch {
      ...
      dreamScheduled = existsSync(
        join(HOME, "Library", "LaunchAgents", "com.skynet-mission-control.dream.plist"),
      );
    }
    if (dreamScheduled) {
      out.push({
...
// memory backends Skynet OS can wire into. Consumed by the setup wizard
...
async function detectApps(): Promise<AppDetections> {
  ...
  // ────────────────────────────────────────────────────────────────────────
  const agIdePath = findAnyMacApp(["Antigravity IDE", "Antigravity"]);
...
async function loadLatestDream() {
  // Reads the most recent ~/.skynet-mission-control/dreams/dream-YYYY-MM-DD.json if it
...
async function main() {
  ...
  if (loadedDream) {
    ...
    console.log(
      `[aggregate] dream: no ~/.skynet-mission-control/dreams/*.json found yet — UI will show sample prescriptions.`,
    );
  }
  ...
  const dreamCronScheduled = (() => {
    if (IS_MACOS) {
      return existsSync(join(HOME, "Library", "LaunchAgents", "com.skynet-mission-control.dream.plist"));
    }
    if (IS_WIN) {
      try {
        const r = Bun.spawnSync(["schtasks", "/query", "/tn", "Skynet Mission Control Dream"]);
        return r.exitCode === 0;
      } catch { return false; }
    }
    return false;
  ...
  if (loadedDream && loadedDream.date) {
    ...
    if (ageDays > 3) {
      ...
      dreamFixHint = "Last dream is more than 3 days old. Check ~/.skynet-mission-control/dream-cron.log.";
    } else {
    ...
    dreamHealthStatus = "silent_failure";
    dreamFixHint = "Check ~/.skynet-mission-control/dream-cron.log for the actual error.";
  }
  ...
  try {
    const dreamStatePath = join(SKYNET_HOME, "dreams", "state.json");
  ...
  // Anthropic's OAuth /usage endpoint already returns 0-100 percentages.
  // A utilization value of 1 means 1% used, not a 0-1 ratio. Multiplying
  // values <= 1 by 100 caused the dashboard's false 100% alarm.
  const auth5hPct = normalizeClaudeUtilization(claudeAuthoritative?.five_hour?.utilization);
  const auth7dPct = normalizeClaudeUtilization(claudeAuthoritative?.seven_day?.utilization);
  const authFableLimit = findClaudeScopedLimit(claudeAuthoritative?.limits, "Fable");
  const authFablePct = normalizeClaudeUtilization(authFableLimit?.percent);
  const claudeWindow = {
    ...
    windows: [
      ...
      {
        label: "Fable",
        used: parsed.familyTurnsWeekly["fable"] || 0,
        cap: weeklyCap,
        pct:
          authFablePct !== undefined
            ? authFablePct
            : Math.min(
                100,
                Math.round(((parsed.familyTurnsWeekly["fable"] || 0) / (weeklyCap || 1)) * 100),
              ),
        source: authFablePct !== undefined ? "oauth" : "estimate",
        resetsAt: authFableLimit?.resets_at ?? claudeAuthoritative?.seven_day?.resets_at ?? null,
      },
    ],
    familyBreakdown: {
  ...
  // Prefer OpenAI's authenticated account endpoint. Hermes is the active OAuth
  // client on this machine, while standalone Codex logs may be absent or stale.
  // Keep the latest Codex log snapshot only as a read-only fallback.
  const liveChatgptLimits = await fetchLiveChatgptUsage();
  const codexFallbackWindows = codexLimits
    ? [
        ...(codexLimits.primary
          ? [
              {
                label: labelChatgptWindow(codexLimits.primary.windowMinutes * 60),
                pct: codexLimits.primary.usedPercent,
                resetsAt: codexLimits.primary.resetsAt,
                windowSeconds: codexLimits.primary.windowMinutes * 60,
              },
            ]
          : []),
        ...(codexLimits.secondary
          ? [
              {
                label: labelChatgptWindow(codexLimits.secondary.windowMinutes * 60),
                pct: codexLimits.secondary.usedPercent,
                resetsAt: codexLimits.secondary.resetsAt,
                windowSeconds: codexLimits.secondary.windowMinutes * 60,
              },
            ]
          : []),
      ]
  ...
  const chatgptLimits =
    ...
    (codexLimits
      ? {
          windows: codexFallbackWindows,
          capturedAt: codexLimits.capturedAt,
          source: "codex-session-log" as const,
        }
      : null);
  ...
  const chatgptWindow = chatgpt.present
    ? {
        ...
        pctUsed: chatgptLimits?.windows[0]?.pct ?? null,
        // An empty or unavailable source is represented as null, never as a
        // fabricated zero. The UI can distinguish unavailable from unused.
        windows: chatgptLimits?.windows.length ? chatgptLimits.windows : null,
        capturedAt: chatgptLimits?.capturedAt ?? null,
        source: chatgptLimits?.source ?? null,
        note:
          chatgptLimits?.source === "openai-account-api"
            ? "Live plan usage from OpenAI's authenticated account endpoint."
            : chatgptLimits?.source === "codex-session-log"
              ? "Fallback rate-limit snapshot from the latest Codex session log."
              : "Usage unavailable — no authenticated live response or local rate-limit snapshot.",
  ...
  // Persist today's snapshot to ~/.skynet-mission-control/history.jsonl and embed the recent
