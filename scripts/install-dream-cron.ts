 ...
 *
 * macOS:
 *   - Writes ~/Library/LaunchAgents/com.skynet-mission-control.dream.plist
 ...
 *     written to ~/.skynet-mission-control/dream-cron.log.
 *
 * Linux / other:
...
const HOME = homedir();
const PLIST_LABEL = "com.skynet-mission-control.dream";
const PLIST_PATH = join(HOME, "Library", "LaunchAgents", `${PLIST_LABEL}.plist`);
const STATE_DIR = SKYNET_HOME;
const DREAM_DIR = join(STATE_DIR, "dreams");
const LOG_PATH = join(STATE_DIR, "dream-cron.log");
...
const WIN_TASK_NAME = "Skynet Mission Control Dream";
...
// as their Dream engine in the dashboard (~/.skynet-mission-control/config.json:
...
// stored at ~/.skynet-mission-control/config.json. We default to Claude Code (the original
...
function readChosenEngine(): "claude" | "hermes" | "codex" {
  try {
    const cfgPath = join(STATE_DIR, "config.json");
...
// chosen engine + model from ~/.skynet-mission-control/config.json at RUNTIME and dispatches
...
function buildPlistXml(hour: number, minute: number): string {
  ...
  // installed (--add-dir ~/.skynet-mission-control + acceptEdits auto-approves the file
...
function warnIfHeadlessAuthMissing() {
  ...
  console.log(`     launchctl kickstart -k gui/$(id -u)/com.skynet-mission-control.dream`);
  console.log(`     ls -la ~/.skynet-mission-control/dreams/           # should contain today's JSON`);
  console.log("");
}
...
function writeWindowsWrapper(): void {
  const lines = [
    ...
    // installed (--add-dir + acceptEdits scope writes to ~/.skynet-mission-control only).
...
function installWindows(hour: number, minute: number, rawTime: string) {
  ...
  const ps = [
    ...
    `Register-ScheduledTask -TaskName '${esc(WIN_TASK_NAME)}' -Action $a -Trigger $t -Description 'Skynet Mission Control daily Dream review' -Force | Out-Null`,
  ].join("; ");
...
function printLinuxFallback(hour: number, minute: number, rawTime: string) {
  ...
  console.log("");
  console.log("  1. Make sure ~/.skynet-mission-control/dreams exists:");
  ...
  console.log("     run-dream.ts reads your chosen engine from ~/.skynet-mission-control/config.json.");
  console.log("");
