#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────
# graph-to-dashboard.sh — graph a repo and register it INTO the Skynet OS
...
if ! curl -sf -o NUL "$BASE/__token" 2>/dev/null; then
  echo "✗ Skynet OS dashboard isn't reachable at $BASE" >&2
  echo "  Start it (bun run dev in ~/code/skynet-mission-control) or set CLAUDE_OS_URL." >&2
  exit 1
fi
...
if d.get('ok'):
    ...
    print('  It now appears in the Skynet OS dashboard gallery + the shared registry.')
