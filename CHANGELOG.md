...
Tracks shipped versions of the Hermes-tier Skynet OS dashboard. Older
versions are kept as zipped artefacts locally for rollback.

---

## Personal v3.1.0 — 29 Jul 2026

Introduces Balcom's **Skynet Mission Control** identity and activates the local command center's Ministry and Graphify integrations. The release is a personal integration on the verified `v3.0.1` baseline, using the official DataSift/REISift documentation bundle supplied for this release.

### Added

- Canonical Skynet Mission Control brand constants and user-visible shell/setup/Oracle branding, while retaining upstream attribution.
- Tested Hermes config resolver that prefers `hermes config path`, then the Windows `LOCALAPPDATA` location, before the legacy home-directory fallback.
- Ministry preset backups plus exact YAML read-back verification and rollback on failure.
- Cross-platform Graphify registry/script path resolution, including Windows backslash support.
- Graphify `0.9.30` activation workflow for Claude Code and the active Hermes profile, with PDF and Office ingestion extras.
- Personalized command-center roadmap covering canonical tasks, iCloud-first calendar integration, approved modules, and adaptive gamification.
- ACSS-aware Paper wireframing contract for Agentic Web Design; low-fidelity layouts now preserve implementation token intent.

### Changed

- Mission Control's bundled Graphify setup script now detects Windows/MSYS, finds `graphify.exe`, uses the active Hermes config directory, and installs the current platform-specific skill.
- Generated Graphify/Hermes prompts now use the real local registry and registration script instead of a hardcoded `~/code/skynet-mission-control` path.

### Safety

- Existing route IDs, local-storage keys, graph IDs, package name, and internal API contracts remain unchanged to avoid data migrations.
- Hermes backups preserve the active config's restrictive file mode; save operations verify disk read-back and restore the backup if verification fails.
- Graphify installation is pinned to reviewed release `0.9.30`, uses supported agent platform names, and handles both POSIX `bin` and Windows `Scripts` virtualenv layouts.
- Existing Opus 4.8 defaults, Sol OAuth fallback, bundled Dream contract, and private/runtime ignore rules are preserved.
- No external state migration or destructive setup was run as part of the source release; Graphify installation remains an explicit operator action.
- The upstream source license and attribution remain intact.

### Verified before release

- `bun test` — 17 passed, 0 failed.
- `bun run check:personal` — all personal release guards passed.
- `bun run build` — production client and SSR builds passed.
- Runtime HTTP verification returned 200 for `/`, `/codegraph`, `/agents/hermes`, and `/setup`; the live registry served `reisift-crm` with 150 nodes and 256 edges.
- Graphify 0.9.30 completed two grounded queries and one three-hop extracted-evidence path against the bundled REISift graph.
- Added-line source and graph scans found no hardcoded secrets, credential-like values, shell injection, dangerous execution, unsafe pickle, or SQL-formatting risks.

### Known limitations

- iCloud Calendar is roadmap-only in this release; no live calendar synchronization is included.
- Sol/Terra/Luna execution depends on each local provider's authenticated configuration and was not asserted by this UI/source release.
- Vite reports existing large-chunk/code-splitting warnings; both production targets still build successfully.

### Rollback

Previous verified release: `v3.0.1`.

---

## Personal v3.0.1 — 26 Jul 2026

Restores the dark operator palette as Skynet OS's default after the v3 upgrade accidentally changed first-run and missing-preference sessions to light mode.

### Fixed

- Dashboard, Memory, Knowledge Graph, sidebar, and shared application shell now default to the same dark palette used by `v2.1.0`.
- An explicitly saved light-mode preference remains supported and is not overwritten.
- Theme initialization now uses one shared contract before first paint and after React hydration, preventing a white flash or conflicting defaults.
- Added regression coverage for missing, dark, light, unknown, and unavailable browser theme storage.

### Verified before release

- Fresh browser profiles rendered Dashboard, Memory, and Knowledge Graph with the `dark` root class and computed `oklch(0.16 0.012 260)` background.
- After-fix screenshots showed readable light text on the restored dark shell across all three affected routes.

---

## Personal v3.0.0 — 26 Jul 2026

Safe integration of the vendor V3.0 source into the personal distribution.

### Upgraded

- Added bounded automatic turn recovery with loop, cost, token, time, child-process, and user-abort guards.
- Added live run status, elapsed/tool indicators, chat reattachment, and explicit failover/resume notes.
- Added model-aware context-window breakdowns and richer markdown/code rendering.
- Added provider-aware lane selection so subscription/OAuth models are not confused with metered duplicates.
- Added model failover, permission/question cards, memory context improvements, and the vendor validation scripts.

### Personal state preserved

- Kept Claude Code on `claude-opus-4-8` by default and as the automatic Claude failover; Opus 5 remains manual-only.
- Kept Codex OAuth fallback on the verified `gpt-5.6-sol` slug rather than invalid bare `gpt-5.6`.
- Preserved the locally repaired bundled Dream contract and 600-second scheduled-run timeout.
- Applied source changes without rerunning setup, replacing `~/.skynet-mission-control`, editing Hermes config/auth, or reinstalling the Windows scheduled task.
- Added an automated personal-release guard and retained install/rollback documentation.

### Verified before release

- Production client and SSR build passed.
- Personal release guard, model-lane suite, turn-watchdog suite, and Dream regression test passed.
- Live browser smoke test rendered the dashboard and Claude picker with zero JavaScript errors.
- External Skynet OS state, Hermes config/environment, and scheduled-task definition matched the pre-upgrade backup after the isolated test server stopped.

See [`docs/UPGRADE-v3.0.0.md`](docs/UPGRADE-v3.0.0.md) for the migration and rollback record.

---

## Personal v2.1.0 — 26 Jul 2026

First immutable baseline of the personal Skynet OS distribution, based on the vendor V2.12 source and including the repaired Windows Dream runner. Build and Dream regression test passed before the baseline tag.
...
- **Windows Dream cron** — Task Scheduler port of the daily Dream review.
  `bun run install-dream` registers a non-elevated `Skynet Mission Control Dream` task that
...
gone. Skynet OS ships as files you own, not a repo you clone.
...
- The Hermes → private-GitHub backup is unchanged — that's _your_ mirror of
  _your_ Hermes, a deliberate feature, not a pointer to the Skynet OS source.
...
- **Grounded graph chat now genuinely queries graphify** (and no longer times
  ...
  channel, so project questions used to deadlock and exit 130 (_"denying
  command"_). The `/codegraph` chat now runs with the **graphify skill +
...
- The shared repo ships with the **`skynet-mission-control` graph only** (the OS graphing
...
- **4 projects ingested** — skynet-mission-control, publish-hub, personal-os, hermes-agent
