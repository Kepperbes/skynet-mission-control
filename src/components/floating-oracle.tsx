  ...
  const localAnswer = useCallback((text: string): string | null => {
    ...
    if (kb.osCode) {
      if (/how big is (the )?(claude ?os|codebase|repo|dashboard|os|this)|how many (files|lines)|size of (the )?(codebase|os|repo)/i.test(t)) {
        return `The Skynet Mission Control codebase is ${kb.osCode.files} files with ${kb.osCode.edges} relationships across ${kb.osCode.modules} modules — straight from its Graphify code graph.`;
      }
