...
import { BrainCircuit, BriefcaseBusiness, Home, LayoutDashboard, LibraryBig, Palette, Radar, Settings as SettingsIcon, Waypoints } from "lucide-react";
import { useEffect, useState } from "react";
import { cn } from "@/lib/utils";
import { APP_ATTRIBUTION, APP_NAME, APP_TAGLINE, UPSTREAM_URL } from "@/lib/brand";
...
const primary = [
  ...
  { to: "/skills", label: "Library", icon: LibraryBig },
];
...
export function AppSidebar() {
  ...
  return (
    <aside className="hidden md:flex w-56 shrink-0 flex-col border-r border-border bg-sidebar sticky top-0 h-screen self-start overflow-y-auto">
      {/* Personal Mission Control identity. Internal storage keys remain stable. */}
      <div className="flex h-14 items-center gap-2.5 px-5">
        <div
          className="relative flex h-8 w-8 items-center justify-center rounded-lg overflow-hidden shrink-0"
          style={{
            background: "linear-gradient(135deg, #60A5FA 0%, #6366F1 55%, #7C3AED 100%)",
            boxShadow:
              "inset 0 1px 0 rgba(255, 255, 255, 0.25), 0 6px 16px -8px rgba(99, 102, 241, 0.75)",
          }}
        >
          <Radar aria-label="Skynet Mission Control" className="h-5 w-5 text-white drop-shadow-sm" />
        </div>
        <div className="flex flex-col leading-tight min-w-0">
          <span className="text-[12.5px] font-semibold tracking-tight truncate">{APP_NAME}</span>
      ...
      <div className="border-t border-border px-4 py-3">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-2 block text-[8px] leading-tight text-muted-foreground/45 hover:text-muted-foreground/80"
        >
          {APP_ATTRIBUTION}
        </a>
      </div>
