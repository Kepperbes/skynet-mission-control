...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard — Skynet Mission Control" },
      {
        name: "description",
        content:
...
interface DreamSuggestion {
  ...
  /** Lifecycle from ~/.skynet-mission-control/dreams/state.json (merged by the aggregator).
...
// Persist a Skip / Mark done / Restore verdict to ~/.skynet-mission-control/dreams/state.json
...
// Real prescriptions from ~/.skynet-mission-control/dreams/dream-{date}.json (inlined by
...
// the currently-picked engine (from ~/.skynet-mission-control/config.json) so the user
...
export function Home({ forceSetupModal = false }: { forceSetupModal?: boolean } = {}) {
  ...
  //      setup` script writes ~/.skynet-mission-control/show-wizard, the middleware reads
  ...
  // drops ~/.skynet-mission-control/show-wizard. The vite middleware reads that file
  ...
  // — this handles the "reinstalled on a laptop that already had Skynet OS"
                        ...
                        <div className="text-[12px] text-violet-200/60 mt-1.5">
                          ...
                          {dreamAgeDays > 3 && (
                            <span className="ml-2 text-amber-300/90">
                              · cron may be failing — check ~/.skynet-mission-control/dream-cron.log
                            </span>
                          )}
                        </div>
                      </>
                    ...
                    return (
                      <div className="rounded-2xl border border-amber-300/25 bg-amber-500/10 p-8 text-center">
                        ...
                        <div className="text-[13px] text-amber-100/80">
                          {dreamFixHint ?? "Check ~/.skynet-mission-control/dream-cron.log for errors."}
                        </div>
                      </div>
                    );
                  }
                        ...
                        className={`text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded-full font-medium shrink-0 ${
                          a.source === "cowork"
                            ...
                            : a.source === "codex"
                              ...
                              : a.source === "skynet-mission-control"
                                ? "bg-violet-500/15 text-violet-400"
                                : "bg-blue-500/15 text-blue-400"
                        }`}
                      >
                        {a.source === "skynet-mission-control" ? "Mission Control" : a.source}
                      </span>
                    )}
                  </div>
...
// Reads ~/.skynet-mission-control name from localStorage and re-renders on storage events
