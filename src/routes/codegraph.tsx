...
import { graphRegistrationScriptFromRegistry, registryPathFromGraphPath } from "@/lib/graph-paths";
...
const CONNECT_PROMPT = `You now have access to my Skynet Mission Control knowledge graphs via graphify.
...
1. Read that index.json first — each entry has an id plus name, language, node/edge counts, communities, and "godNodes" (the most-connected files). Locally ingested entries may also have an absolute "graphPath".
2. Resolve the graph from graphPath when present; otherwise use \`<registry-directory>/<id>.json\`. Reason from that graph or run graphify directly: \`graphify query "<question>"\`, \`graphify path "A" "B"\`, \`graphify explain "X"\`.
3. The legacy "claude-os" entry is Skynet Mission Control itself — use it to answer questions about the command center.
...
When I ask you to graph a NEW repo (one not already in the registry), register it through the dashboard so it appears in my gallery:
  bash "{{SCRIPT}}" <local-path-or-github-url>
...
function CodeGraphPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <ConnectHermesCard
        registryPath={
          registryPathFromGraphPath((active as any).graphPath as string | undefined) ??
...
function ConnectHermesCard({ registryPath, accent }: { registryPath: string; accent: string }) {
  ...
  const prompt = CONNECT_PROMPT.replace("{{REGISTRY}}", registryPath).replace(
    "{{SCRIPT}}",
    graphRegistrationScriptFromRegistry(registryPath),
  );
