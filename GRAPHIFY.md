...
Skynet OS wires it into **one shared memory** across the dashboard, Hermes,
and Claude Code.
...
- The `skynet-mission-control` entry is the OS graphing itself, so the agents can
  reason about Skynet OS's own structure.
